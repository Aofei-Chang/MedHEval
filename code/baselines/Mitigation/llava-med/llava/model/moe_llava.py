...
class LoRA_MOE_QK_old(nn.Module): # for Q,K,V,O in attention
    ...
    def forward(self, x):
        ...
        # x_question_part = x[:, question_start_index:, :]      # Qu
        # non_question_out = self.original_module(x_non_question_part)
        # logits = self.router(x_question_part)
        ...
        # moe_out = self.forward_lora_moe_sparse(x_question_part, self.original_module, index, self.moe)
        ...
        if x.size(1) > 1:  # Initial forward pass
            ...
            x_non_image_part1 = x[:, image_start_index+256:, :]  # Non-question part (before question_start_index)
            x_image_part = x[:, image_start_index:image_start_index+256, :]      # Qu
