...
for baseline in "${baselines[@]}"; do
    python llava/eval/eval_batch.py --num-chunks 4  --model-name ${ROOT_PATH}/LLM/llava_med \
        ...
        --answers-file ${ROOT_PATH}/hallucination/MedHEval/type1/baselines/${dataset}/pred_${baseline}.jsonl \
        --baseline ${baseline}
    wait
done
...
for baseline in "${baselines[@]}"; do
    python llava/eval/eval_batch.py --num-chunks 4  --model-name ${ROOT_PATH}/LLM/llava_med \
        ...
        --image-folder ${ROOT_PATH}/hallucination/${dataset}/images \
        --answers-file ${ROOT_PATH}/hallucination/MedHEval/type1/baselines/${dataset}_closed/pred_${baseline}.jsonl \
        --baseline ${baseline}
    wait
done
...
for baseline in "${baselines[@]}"; do
    python llava/eval/eval_batch.py --num-chunks 4  --model-name ${ROOT_PATH}/LLM/llava_med \
        ...
        --image-folder ${ROOT_PATH}/hallucination/${dataset}/images \
        --answers-file ${ROOT_PATH}/hallucination/MedHEval/type2/baselines/${dataset}_closed/pred_${baseline}.jsonl \
        --baseline ${baseline}
    wait
done
